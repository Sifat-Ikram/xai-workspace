...
export default function InsightFlow() {
  ...
  const jumpToStage = (index) => {
    ...
    if (!st) return;
    const target =
      st.start + (st.end - st.start) * (index / (stages.length - 1));
    gsap.to(window, {
      duration: 1,
      scrollTo: { y: target },
      ease: "power2.inOut",
  ...
  return (
    <section
      id="workflow"
      ref={containerRef}
      className="relative h-screen overflow-hidden bg-surface border-y border-border"
    >
      <svg
        ref={lineRef}
        viewBox="0 0 1000 4"
      ...
      >
        {stages.map((stage, i) => (
          <div
            key={stage.tag}
            className="insight-panel group relative h-full shrink-0 flex items-center justify-center px-6 md:px-24"
            style={{ width: "100vw" }}
          >
            <div
                ...
                {stages.map((_, dotIndex) => (
                  <button
                    ...
                    style={{
                      ...
                      backgroundColor:
                        dotIndex === i ? stage.stroke : "#1E2530",
                    }}
                  />
                ))}
              </div>
